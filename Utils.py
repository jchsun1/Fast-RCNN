...
if __name__ == "__main__":
    # 在17flowers数据集上进行预训练
    train_root = "./data/ss"
    myset = GenDataSet(root=train_root, batch_size=2)
    for n in range(3):
        for batch_index, batch in enumerate(myset):
            use = batch[0]
            name = use[0]
            img = use[1]
            ss_boxes = use[5]
    ...
    #         draw_box(img, img0, ss_boxes, gt_boxes, ss_boxes0, gt_boxes0)

    path = r"D:\WorkSpace\04-ObjDetect\MyFastRCNN\data\ss\image_0561\image_0561.jpg"
    im = io.imread(path)
    print(im.shape)
    im_t = np.transpose(im, (2, 0, 1))
    print(im_t.shape)
    tt = torch.tensor(np.transpose(im, (2, 0, 1)))
    print(tt.size())
    exit(0)
    path = r"D:\WorkSpace\04-ObjDetect\MyFastRCNN\data\ss\image_0561\ss_loc.csv"
    loc = pd.read_csv(path, header=0, index_col=None).values
    ...
    for line in loc:
        ...
        rect = patches.Rectangle((int(gt[0]), int(gt[1])), int(gt[2]), int(gt[3]), linewidth=1, edgecolor='g', facecolor='none')
        axis1.add_patch(rect)

        rows, cols = im.shape[:2]
        im2 = cv.resize(src=im, dsize=(width, height), interpolation=cv.INTER_CUBIC)
        # 计算resize过程中x,y方向尺度缩放比例
        scale_ratio = np.array([width / cols, height / rows, width / cols, height / rows])
        box = (box * scale_ratio).astype("int")
        gt = (gt * scale_ratio).astype("int")
        print(box)
        print(gt)
        plt.subplot(122)
        plt.imshow(im2)
